...
function joinSession() {
  ...
  session.on('connectionCreated', event => {
    appendEvent({ event: 'connectionCreated', content: event.connection.connectionId, date: Date.now() });
  });

  session.on('streamCreated', event => {
    appendEvent({ event: 'streamCreated', content: event.stream.streamId, date: Date.now() });
  ...
  session.on('streamDestroyed', event => {
    appendEvent({ event: 'streamDestroyed', content: event.stream.streamId, date: Date.now() });
    var userId = event.stream.connection.data;
    document.getElementById('video-' + userId).outerHTML = '';
  });

  session.on('sessionDisconnected', event => {
    appendEvent({ event: 'sessionDisconnected', content: session.connection.connectionId, date: Date.now() });
...
function insertPublisherContainer() {
  var commonTagStyle =
    "display: inline-block; cursor: pointer; background-color: #daae00; color: white; font-size: 13px; font-weight: bold; padding: 1px 3px; border-radius: 3px; font-family: 'Arial';";
...
function setPublisherButtonsActions(publisher) {
  ...
  document.getElementById('unpublish').onclick = () => {
    if (event.target.innerText === 'Unpublish') {
      ...
      var publisher2 = OV.initPublisher(insertPublisherContainer(), {
        resolution: '540x360',
        frameRate: 30,
        mirror: false,
      });
  ...
  publisher.once('accessAllowed', e => {
    appendEvent({ event: 'accessAllowed', content: '', date: Date.now() });
  });
  publisher.once('streamCreated', e => {
    appendEvent({ event: 'streamCreated', content: e.stream.streamId, date: Date.now() });
  });
  publisher.once('streamPlaying', e => {
    appendEvent({ event: 'streamPlaying', content: 'Publisher', date: Date.now() });
    window.openviduLoadTest.stats[USER_ID] = [];
  });
  publisher.once('streamDestroyed', e => {
    appendEvent({ event: 'streamDestroyed', content: e.stream.streamId, date: Date.now() });
...
function insertSubscriberContainer(event) {
  var commonTagStyle =
    "background-color: #0088aa; color: white; font-size: 13px; font-weight: bold; padding: 1px 3px; border-radius: 3px; font-family: 'Arial'";
  var videoContainer = document.createElement('div');
...
function collectEventsAndStats() {
  this.session.streamManagers.forEach(streamManager => {
    this.gatherStatsForPeer(
      streamManager.stream.getRTCPeerConnection(),
      streamManager.stream.connection.data,
      streamManager.remote,
    );
  });
}
...
function gatherStatsForPeer(rtcPeerConnection, userId, isSubscriber, errorCallback) {
  return rtcPeerConnection.getStats(
    response => {
      const fullReport = [];
      ...
      var activeCandidateStats = fullReport.find(
        report => report.type === 'googCandidatePair' && report.googActiveConnection === 'true',
      );
      ...
      if (!!videoBwe) {
        if (isSubscriber) {
          ...
          document.querySelector('#receive-bandwidth-' + userId).innerText =
            'Bandwidth: ' + userStatsJson.availableReceiveBandwidth + ' kbps';
        } else {
          ...
          document.querySelector('#send-bandwidth').innerText =
            'Bandwidth: ' + userStatsJson.availableSendBandwidth + ' kbps';
      ...
      if (!!videoStats) {
        ...
        if (isSubscriber) {
          if (!lastBytesReceived[userId]) {
            ...
            userStatsJson.bitrate = Math.floor(
              (Number(videoStats.bytesReceived) * 8) / (videoStats.timestamp.getTime() - lastStatGatheringTime[userId]),
            );
          } else {
            userStatsJson.bitrate = Math.floor(
              ((Number(videoStats.bytesReceived) - lastBytesReceived[userId]) * 8) /
                (videoStats.timestamp.getTime() - lastStatGatheringTime[userId]),
            );
          }
  ...
  if (streamId && window.session.streamManagers) {
    ...
    for (let streamManager of window.session.streamManagers) {
      if (streamManager && streamManager.stream && streamManager.stream.streamId === streamId) {
        ...
        console.log(
          'Local recorder from streamId "' + streamId + '" has been initialized with ID: ' + localRecorder.id,
        );
